...
#if defined(CYW20721B2) || defined(CYW43012C0) || defined(CYW55500A1)
...
static void hfp_retry_conn_handler(TIMER_PARAM_TYPE param )
{
    if( handsfree_app_states.may_retry_conn > 0 )
    {
        wiced_bt_hfp_hf_connect(handsfree_app_states.retry_bdaddr);
        handsfree_app_states.may_retry_conn--;
        WICED_BT_TRACE("%s: Retry connection to HFP [%B] \n", __func__, handsfree_app_states.retry_bdaddr);
    }
}
...
static void handsfree_connection_event_handler(wiced_bt_hfp_hf_event_data_t* p_data)
{
    ...
    if(p_data->conn_data.conn_state == WICED_BT_HFP_HF_STATE_CONNECTED)
    {
        ...
        handsfree_app_states.may_retry_conn = 0;
        if( wiced_is_timer_in_use(&handsfree_app_states.retry_conn_timer) )
        {
            wiced_stop_timer(&handsfree_app_states.retry_conn_timer);
        }
    ...
    else if(p_data->conn_data.conn_state == WICED_BT_HFP_HF_STATE_DISCONNECTED)
    {
        ...
        if(handsfree_app_states.may_retry_conn > 0)
        {
            ...
            memcpy(handsfree_app_states.retry_bdaddr, p_data->conn_data.remote_address, WICED_BT_ADDRESS_BYTE_SIZE);
            wiced_start_timer(&handsfree_app_states.retry_conn_timer,2000);
            WICED_BT_TRACE("%s: Retry connection to HFP [%B] \n", __func__, p_data->conn_data.remote_address);
        }
        else
        {
...
void hci_control_hf_init(void)
{
    ...
    wiced_init_timer(&handsfree_app_states.retry_conn_timer, hfp_retry_conn_handler, 0, WICED_MILLI_SECONDS_TIMER);
    handsfree_hfp_init();

#ifdef CYW55500A1
    // SCO voice path PCM
    {
        wiced_bt_voice_path_setup_t handsfree_voice_path;
        handsfree_voice_path.path = WICED_BT_SCO_OVER_PCM;
        handsfree_voice_path.p_sco_data_cb = NULL;
#if defined(SUPPORT_MXTDM)
        ...
        wiced_bt_sco_setup_voice_path(&handsfree_voice_path);
    }
#endif
}
...
void hci_control_hf_handle_command(uint16_t opcode, uint8_t* p_data, uint32_t length)
{
    ...
    case HCI_CONTROL_HF_COMMAND_CONNECT:
        ...
        if(WICED_SUCCESS == wiced_bt_hfp_hf_connect(bd_addr))
        {
            if( wiced_is_timer_in_use(&handsfree_app_states.retry_conn_timer) )
            {
                wiced_stop_timer(&handsfree_app_states.retry_conn_timer);
            }
            handsfree_app_states.may_retry_conn = 2;
            WICED_BT_TRACE("%s: Connection to HFP [%B] \n", __func__, bd_addr);
        }
        break;
    ...
    switch ( event )
    {
        case BTM_SCO_CONNECTED_EVT:             /**< SCO connected event. Event data: #wiced_bt_sco_connected_t */
            ...
            if (WICED_SUCCESS != wiced_am_stream_set_param(stream_id, AM_MIC_GAIN_LEVEL, (void *) &audio_config.mic_gain))
                WICED_BT_TRACE("wiced_am_set_param failed\n");
#endif /* CYW20721B2 || CYW43012C0 || CYW55500*/
