...
wiced_result_t avrc_app_hci_connect ( uint8_t* p_data, uint32_t len )
{
    ...
    result = wiced_bt_avrc_ct_connect( bd_addr );

    if((result == WICED_BT_SUCCESS)  || (result == WICED_PENDING))
    {
        if( wiced_is_timer_in_use(&rc_app_cb.retry_conn_timer) )
        {
            wiced_stop_timer(&rc_app_cb.retry_conn_timer);
        }
        rc_app_cb.may_retry_sdp = 2;
    }
                                  ...
                                  uint32_t peer_features)
{
    ...
    case REMOTE_CONTROL_DISCONNECTED:
        ...
        if(((wiced_result_t)WICED_BT_SDP_CONN_FAILED == status) && (rc_app_cb.may_retry_sdp > 0))
        {
            if( wiced_is_timer_in_use(&rc_app_cb.retry_conn_timer) )
            {
                wiced_stop_timer(&rc_app_cb.retry_conn_timer);
            }
            memcpy(rc_app_cb.retry_bdaddr, remote_addr, WICED_BT_ADDRESS_BYTE_SIZE);
            wiced_start_timer(&rc_app_cb.retry_conn_timer,2000);
            WICED_BT_TRACE("%s: Retry connect to AVRC ct [%B] \n", __func__, remote_addr);
        }
        else
        {
...
static void hci_control_rc_retry_conn_handler(TIMER_PARAM_TYPE param )
{
    if( rc_app_cb.may_retry_sdp > 0 )
    {
        wiced_bt_avrc_ct_connect(rc_app_cb.retry_bdaddr);
        rc_app_cb.may_retry_sdp--;
        WICED_BT_TRACE("%s: Retry connection to AVRC CT [%B] \n", __func__, rc_app_cb.retry_bdaddr);
    }
}
...
void hci_control_rc_controller_init( void )
{
    ...
    wiced_init_timer(&rc_app_cb.retry_conn_timer, hci_control_rc_retry_conn_handler, 0, WICED_MILLI_SECONDS_TIMER);
