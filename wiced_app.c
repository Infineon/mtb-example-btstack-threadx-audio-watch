...
#ifndef NO_PUART_SUPPORT
#define PUART_LOG_BUF_LEN 255
char g_puart_log_buf[PUART_LOG_BUF_LEN];
...
#ifndef NO_PUART_SUPPORT
void wiced_app_puart_print(char* p_trace_buf, int trace_buf_len, wiced_bt_trace_type_t trace_type)
{
    if(p_trace_buf)
    {
        p_trace_buf[trace_buf_len-1] = 0;
        wiced_hal_puart_print(p_trace_buf);
    }
}

void wiced_app_enable_puart_hw(void)
{
    ...
    cy_retarget_io_init(LHL_GPIO_2, LHL_GPIO_3, 3000000);
    wiced_bt_dev_register_debug_trace(wiced_app_puart_print, g_puart_log_buf, PUART_LOG_BUF_LEN);
}
#endif
...
#elif defined CYW55500A1
    wiced_app_enable_puart_hw();
#else /* !CYW55572 */
    wiced_hal_puart_init();
